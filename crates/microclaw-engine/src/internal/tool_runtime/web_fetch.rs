...
use crate::internal::tool_runtime::web_content_validation::{
    validate_web_content_with_config, WebContentValidationConfig,
};
use crate::internal::tool_runtime::web_html::{extract_primary_html, html_to_text};
...
pub fn validate_web_fetch_url(
    ...
    if config.block_private_ips {
        let allowlisted = config
            ...
            .any(|rule| host_matches_rule(&host, rule));
        if !allowlisted {
            crate::internal::tool_runtime::url_safety::check_url_private_ip(&parsed)?;
        }
    }
